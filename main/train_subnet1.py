# train_subnet1.py
"""
Training script for Sub-network 1 (EfficientNet-based survival model).
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Sub-network 1 (EfficientNet survival model)")

    # GPU settings
    parser.add_argument("--gpu", type=str, default="1", help="CUDA_VISIBLE_DEVICES id list, e.g. 1 or 1,2,3")
    ...
    parser.add_argument("--drop_rate", type=float, default=0.0)  # kept for compatibility
    parser.add_argument("--rand_p", type=float, default=0.45)
    ...
    parser.add_argument("--skip_epoch_model", type=int, default=25)
...
def _try_model_summary(model: torch.nn.Module, input_tensor_shape):
    """
    torchinfo is optional. If not installed, just skip.
    """
    try:
...
def main():
    ...
    # Optional: model summary (rank0 only)
    if _is_rank0(rank):
        try:
    ...
    t = trange(args.max_epochs, desc="EfficientNet survival -- epoch 0, avg loss: inf", leave=True) if _is_rank0(rank) else range(args.max_epochs)

    for epoch in t:
        # DDP: set epoch for sampler to shuffle differently each epoch
        ...
        # collect for epoch metrics (rank0 only)
        train_OS = None
        train_OStime = None
        train_OSpred = None
        train_PFS = None
        ...
        for batch_data in train_loader:
            ...
            # rank0: collect for epoch-level metrics (same behavior as original)
            if _is_rank0(rank):
                if step == 0:
                    train_OS = t_OS
            ...
            # L1 regularization (use base model parameters)
            l1_reg = None
        ...
        # validation/test (rank0 only, to keep behavior identical)
        if _is_rank0(rank) and ((epoch + 1) % 1 == 0):
            model.eval()
            with torch.no_grad():
                # ------------------- validation ------------------- #
                val_OS = None
                ...
                metric2 = s_cindex_OS
                print("metric1_s_cindex_PFS", metric1)
                print("metric2_s_cindex_OS", metric2)
                ...
                if epoch > args.skip_epoch_model:
                    ...
                    if metric1 > best_metric_pfs:
                        ...
                        torch.save(base_model.state_dict(), args.best_model_name + "_CV42_PFS.pth")

                    if metric2 > best_metric_os:
                        best_metric_os = metric2
                        best_metric_os_epoch = epoch + 1
                        torch.save(base_model.state_dict(), args.best_model_name + "_CV42_OS.pth")
                    ...
                    print(
                        ...
                        f"\n best loss: {best_metric_os:.4f} at epoch {best_metric_os_epoch}"
                    )
