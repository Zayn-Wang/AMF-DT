# train_subnet5.py
"""
Training script for Sub-network 5 (M3t-based survival model).

This refactors the original monolithic script:
- uses models_m3t.M3t as backbone
- uses shared losses / metrics / data_utils
- trains with multi-task loss (PFS, Age, Clinical labels) + OS loss + L1 regularization
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train Sub-network 5 (M3t survival model).")

    # GPU
    parser.add_argument("--gpu", type=str, default="1", help="CUDA_VISIBLE_DEVICES id list, e.g. 0 or 0,1,2")
    ...
    parser.add_argument("--rand_p", type=float, default=0.3)
    ...
    parser.add_argument("--n_loss", type=int, default=3)

    # DataLoader workers
    parser.add_argument("--num_workers_train", type=int, default=3)
    parser.add_argument("--num_workers_val", type=int, default=3)
    parser.add_argument("--num_workers_test", type=int, default=3)
...
def main():
    ...
    loss_func = MultiTaskLossWrapper(args.n_loss).to(device)
    ...
    # visualize NET (rank0 only; torchinfo optional)
    if _is_rank0(rank):
        try:
    ...
    t = trange(args.max_epochs, desc="M3t survival -- epoch 0, avg loss: inf", leave=True) if _is_rank0(rank) else range(args.max_epochs)
    ...
    for epoch in t:
        ...
        for batch_data in train_loader:
            ...
            t_label = torch.cat(
                (t_Gender, t_Child_Pugh, t_HBV, t_LMet, t_AMet, t_BoMet, t_BrMet, t_LNMet, t_stage),
                dim=1,
            )
            ...
            # rank0: accumulate for epoch metrics
            if _is_rank0(rank):
                if step == 0:
                    train_OS = t_OS
                    ...
                    train_label = torch.cat((train_label, t_label), dim=0)
                    train_labelpred = torch.cat((train_labelpred, output4), dim=0)

            # L1 regularization (use base model params to be safe)
            l1_reg = None
            ...
            # same as original: multi-task(PFS, Age, Label) * log10(L1) * OS
        ...
        if _is_rank0(rank) and ((epoch + 1) % val_interval == 0):
            ...
            with torch.no_grad():
                ...
                for val_data in val_loader:
                    ...
                    v_label = torch.cat(
                        (v_Gender, v_Child_Pugh, v_HBV, v_LMet, v_AMet, v_BoMet, v_BrMet, v_LNMet, v_stage),
                        dim=1,
                    )
                    ...
                    if val_step == 0:
                        ...
                        val_label = torch.cat((val_label, v_label), dim=0)
                        val_labelpred = torch.cat((val_labelpred, val_output4), dim=0)
                ...
                for test_data in test_loader:
                    ...
                    s_label = torch.cat(
                        (s_Gender, s_Child_Pugh, s_HBV, s_LMet, s_AMet, s_BoMet, s_BrMet, s_LNMet, s_stage),
                        dim=1,
                    )
                    ...
                    if test_step == 0:
                        ...
                        test_label = torch.cat((test_label, s_label), dim=0)
                        test_labelpred = torch.cat((test_labelpred, s_output4), dim=0)
                ...
                if epoch > args.skip_epoch_model:
                    ...
                    if metric_pfs > best_metric_pfs:
                        ...
                        torch.save(base_model.state_dict(), args.best_model_name + "_CV31_PFS.pth")

                    if metric_os > best_metric_os:
                        best_metric_os = metric_os
                        best_metric_os_epoch = epoch + 1
                        torch.save(base_model.state_dict(), args.best_model_name + "_CV31_OS.pth")
